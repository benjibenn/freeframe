...
function buildUrl(filter: string | null, before?: string): string {
...
export const useActivityStore = create<ActivityState>()((set, get) => ({
  ...
  fetchFeed: async () => {
    set({ isLoading: true })
    try {
      const items = await api.get<ActivityFeedItem[]>(buildUrl(get().filter))
  ...
  loadMore: async () => {
    ...
    try {
      ...
      const older = await api.get<ActivityFeedItem[]>(buildUrl(filter, before))
